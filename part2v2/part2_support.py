...
def recover_part1_password(student_id: str) -> str:
    ...
    for index in range(length):
        ...
        state = lcg_next(state)
        _high = state
        out[index] = vault[row_index][col_val]
    return "".join(out)
...
def validate_part1_password(student_id: str, password: str) -> bool:
    return normalize_password(password) == recover_part1_password(student_id)
...
def generate_mission(student_id: str, password: str | None = None) -> Dict[str, object]:
    expected_password = recover_part1_password(student_id)
    mission_password = expected_password if password is None else normalize_password(password)
    if mission_password != expected_password:
        raise ValueError("Incorrect Part 1 password for this student ID.")

    width = 8
    height = 8
    state = derive_mission_seed(student_id, mission_password)
    snake_x = [2, 1, 0]
    snake_y = [4, 4, 4]
    planned_x = snake_x[:]
    planned_y = snake_y[:]
    ...
    while step_number < 16:
        ...
        _, move, planned_x, planned_y, _ = choice
        moves.append(move)
